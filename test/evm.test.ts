...
describe('EVM Provider (ETH/BSC/POL)', () => {
  ...
  it('should return merged coin + token transactions for ETH', async () => {
    ...
    expect(txs[0].value).toBe('50000000'); // raw, client does toLiteral
    expect(txs[0].decimals).toBe(6);
    expect(txs[0].tokenTransfers).toHaveLength(1);
    expect((txs[0].tokenTransfers as Array<Record<string, unknown>>)[0].value).toBe('50000000');
    ...
    expect(txs[1].value).toBe('1000000000000000000'); // raw, client does toLiteral
